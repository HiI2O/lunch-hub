...
#### アクセス制御（役割ベース）
- **GENERAL_USER（一般ユーザー）**: 自分の予約のみ管理
- **STAFF（係）**: 全ユーザーの予約確認・管理、ゲスト予約作成
- **ADMINISTRATOR（管理者）**: 係の権限 + ユーザー管理
...
#### 予約締め切り（自動）
- **当日9:30で予約を自動締め切り**: 9:30以降は当日の予約の作成・変更・キャンセルが不可となる
- ドメインルールとしてサーバー側で現在時刻をチェックし制御（スケジューラー不要）

#### 注文確定（手動）
- 係は9:30以降に当日の予約一覧を確認し、お弁当屋さんに電話で手動発注する
- 発注後、係がシステム上で注文を「発注済み」に変更
...
- 無効化時に全セッションを破棄する（即座にログアウト）
- 無効化時に未来の予約を自動キャンセルする
- 無効化されたユーザーのチケット残高はデータとして保持するが、予約には使用できない
- 管理者は自分自身を無効化できない
- 無効化の用途: 異動・退職時のアカウント停止
- 無効化されたユーザーを再有効化可能（ステータス: ACTIVE）
- 再有効化時はパスワード再設定が必要（招待メールと同様のフローで再アクティベーション）
...
#### ログ記録（監査）
- 認証に関わる全操作（ログイン成功・失敗、招待、アクティベーション）を30日間保持
- パスワード変更のログ記録
...
### 6.2 ビジネス的制約
- 社内メンバーのみがアクセス可能(招待制)
- **提供される弁当は「日替わり弁当」の1種類のみ**。メニュー選択の機能は不要
- 弁当の価格や種類の管理は対象外(固定)
