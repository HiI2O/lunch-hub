...
- MUST: `.env` ファイルを読み取らない（機密情報が含まれるため）
- MUST: パスワード・トークン・シークレットをコードにハードコードしない
- MUST: `.env` の内容をコミットメッセージ・PR本文・コメントに含めない
- MUST: 環境変数の参照が必要な場合は `.env.example` を見る（こちらにはプレースホルダーのみ）
